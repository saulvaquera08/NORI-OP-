...
export function FormuladorScreen({
  ...
  return (
    <div className="flex h-full flex-col">
      {/* ---- barra de recetas ---- */}
      <div className="flex flex-wrap items-center gap-2 border-b border-nori-border px-6 py-3">
        {recipes.map((r) => (
          <Link
            key={r.id}
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="min-w-0 flex-1 overflow-y-auto p-6 pt-5">
          <div className="mb-1 flex flex-wrap items-center gap-[10px]">
            ...
            {editable && (
              <button
                ...
                className="ml-auto rounded-lg bg-nori-terracota px-3 py-[7px] text-[12px] font-semibold text-nori-terracota-deep disabled:opacity-50"
              >
                {isSaving ? "Guardando…" : `Guardar como V${Math.max(...versions.map((v) => v.versionNumber)) + 1}`}
              </button>
            )}
          </div>
          {saveError ? <div className="mb-2 text-[12px] text-nori-red">{saveError}</div> : null}
          ...
          <div className="flex flex-col gap-[6px] overflow-x-auto">
            ...
            {rows.length === 0 ? (
              <div className="rounded-[10px] border border-dashed border-white/[0.12] p-4 text-[12.5px] text-nori-text-dim">
                Agrega ingredientes del catálogo con el botón +.
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="w-[228px] flex-none overflow-y-auto border-l border-nori-border p-[14px] pt-5">
          ...
          <div className="mb-4 grid grid-cols-2 gap-2">
            ...
            <div className="rounded-[10px] border border-nori-border bg-nori-card p-3">
              <div className="text-[10.5px] text-nori-text-muted">Costo / {porcionLabel}</div>
          ...
          <div className="mt-2 text-[10px] leading-relaxed text-nori-text-dim">
            Estimado con macros de tabla estándar — pendiente bromatológico. Las kcal excluyen carbohidratos
